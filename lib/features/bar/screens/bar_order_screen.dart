  ...
  Future<void> _updateItemStatus(String itemId, String newStatus) async {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (mounted) {
          _showNotification('Order updated to $newStatus'); // ✅ NOVO
        }
      } else {
        if (mounted) {
          _showNotification(
...
class _BarOrderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isUrgent = item.createdAt != null &&
        DateTime.now().difference(item.createdAt!).inMinutes > 10;
